...
Standard MCP client configuration import reads `mcpServers`; a multi-server file lets you select one entry. Export can produce either an Open Science Connector or portable MCP client JSON. Secrets and headers are replaced with `${NAME}` placeholders. Pre-registered OAuth uses an optional client ID and write-only client secret; changing its issuer or client ID invalidates the stored authorization. Device credentials can be bound by name instead of copied into the Connector.
