...
## 更新后的服务商目录 {/* #provider-catalog-updates */}

v0.33.0 目录加入 **Xiaomi MiMo v2.6**、**xAI Grok 4.7**，并更新 **OpenCode Zen** 和 **Go**。新建 MiMo 配置默认使用 `mimo-v2.6-pro`，原有 v2.5 选项仍保留。Grok 4.7 成为 xAI 的新默认模型，旧模型 ID 仍在列表中。

选择当前 Agent 实际提供的模型，检查输入和推理选项，再发送一个小请求。目录默认值不会更改所有已有会话的模型，也不保证账号拥有使用权限。主会话可以保留 Codex，另行配置[分类服务](models.md#classification-models)。

## 新增模型目录选项 {/* #catalog-models */}
