...
Откройте для себя активные записи белка UniProtKB по точному названию гена (включая синонимы), фразе белкового имени и / или точному organism_id (идентификатор таксономии NCBI, а не потомков). Требуется хотя бы один из этих фильтров. Поставляемые фильтры сочетаются с И. Optional review=true selects Swiss-Prot, false selects TrEMBL; Опускание включает в себя оба. Никакого организма или пересмотренного по умолчанию. Текст использует токенизированный синтаксис фразы UniProt, а не произвольное подстрочное соответствие или синтаксис необработанного запроса; Цитаты, обратные удары, wildcards и контрольные символы отвергаются. Возвращает одну ограниченную страницу в порядке присоединения, а не полный набор белков. На следующей странице перейдите next_cursor в виде курсора с идентичными фильтрами и page_size. Курсоры непрозрачны, не смещены и не долговечны; Перезагрузка, если UniProt отклонит устаревший курсор.

Поставить хотя бы один поисковый фильтр; Проконсультируйтесь с загружаемой схемой для полного набора правил.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `gene` | строка | факультативный; Длина: `1`; Максимальная длина: `200`; шаблон: `"^(?=[\\s\\S]*\\S)[^\"\\\\*?\\u0000-\\u001f\\u007f]+$"` |
| `protein_name` | строка | факультативный; Длина: `1`; Максимальная длина: `200`; шаблон: `"^(?=[\\s\\S]*\\S)[^\"\\\\*?\\u0000-\\u001f\\u007f]+$"` |
| `organism_id` | целое число | факультативный; минимум: `1`; максимум: `2147483647` |
| `reviewed` | логическое значение | строка |
| `page_size` | целое число | необязательный `25`; минимум: `1`; максимум: `500` |
| `cursor` | строка | факультативный; Длина: `1`; Максимальная длина: `4096`; шаблон: `"^[^\\s\\u0000-\\u001f\\u007f]+$"` |
...
Отправляйте до идентификаторов 100,000 для картирования пакетных идентификаторов UniProt. from_db/to_db - это точные имена баз данных UniProt API (например, Gene_Name, GeneID, Ensembl, RefSeq_Protein, UniProtKB_AC-ID -> UniProtKB; UniProtKB_AC-ID -> Ensembl или GeneID. Действительные пары определяются [https://rest.uniprot.org/configure/idmapping/fields](https://rest.uniprot.org/configure/idmapping/fields); Неподдерживаемые пары выходят из строя. taxon_id является опциональным только для Gene_Name. Укажите его для разбора видов. Идентификаторы должны быть отдельными строками без белого пространства или разделителей. Случай и версии сохранены, точные дубликаты представлены один раз. Перейдите из search_uniprot_entries в качестве идентификаторов с from_db=UniProtKB_AC-ID. Отправляет один POST, никогда автоматически не перезагружает и не проводит опросы. Сохраните job_id, затем используйте get_uniprot_id_mapping_status и get_uniprot_id_mapping_results. У UniProt истекает срок действия результатов через 7 дней; Отмена или отключение приложения останавливает локальные запросы, а не удаленную работу. Не предоставляется локальный кэш-память или удаленная отмена / удаление API. Если подчинение теряет свой ответ, работа может существовать. Не возвращайтесь вслепую.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `from_db` | строка | **обязательно**; Длина: 1; Максимальная длина: 100; модель: "^&#91;A-Za-z&#93;&#91;A-Za-z0-9_-&#93;&#42;$" |
| `to_db` | строка | **обязательно**; Длина: 1; Максимальная длина: 100; модель: "^&#91;A-Za-z&#93;&#91;A-Za-z0-9_-&#93;&#42;$" |
| `ids` | массив строк | **обязательно**; Мини-элементы: 1; maxItems: 100000 |
| `taxon_id` | целое число | факультативный; Минимум: 1; Максимум: 2147483647 |
...
Проверьте существующую карту UniProt ID. NEW/RUNNING означает опрос этого инструмента позже (с интервалом не менее 3 секунд); FINISHED означает, что вы можете получить все страницы с get_uniprot_id_mapping_results. Upstream ERROR нормализуется как FAILED, терминальный сбой в работе, а не непревзойденные идентификаторы. HTTP 400/500 сбои в работе считываются без автоматических повторных попыток; Другие неудачи HTTP (включая неизвестные / просроченные рабочие места) распространяются. Не представляет новую работу или автоматический опрос.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Длина: 1; Максимальная длина: 100; модель: "^&#91;A-Za-z0-9_-&#93;+$" |
...
Возьмите одну страницу компактных картографических пар UniProt ID для готовой работы. Повторять с next_cursor и идентичными job_id/page_size до has_more=ложно; Курсоры непрозрачны, не смещены и не долговечны. Сохраняется каждая строка от / до, включая отображения от одного до многих. Слияние пар на всех страницах перед интерпретацией множественности; Источник может охватывать страницы. Собрать объединение failed_ids, представленное на разных страницах; Никогда не делайте вывод о непревзойденных идентификаторах из отсутствия на странице. Идентификаторы целей UniProtKB могут быть переданы get_uniprot_entries для аннотаций или последовательностей.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Длина: 1; Максимальная длина: 100; модель: "^&#91;A-Za-z0-9_-&#93;+$" |
| `page_size` | целое число | факультативный; по умолчанию: 100; Минимум: 1; Максимум: 500 |
| `cursor` | строка | факультативный; Длина: 1; Максимальная длина: 4096; шаблон: "^&#91;^\\\\u0000-\\u001f\\\u007f&#93;+$" |
...
| Полное поле | Тип | Требования и ограничения |
| --- | --- | --- |
| `organism` | строка | **обязательно**; Длина: `1`; Максимальная длина: `64`; шаблон: `"^[a-z][a-z0-9_]*$"` |
...
Отправьте одну нуклеотидную или белковую последовательность в общедоступную службу NCBI BLAST для поиска асинхронного сходства. Установите molecule_type явно, потому что белок, сделанный только из A/C/G/T, в противном случае неоднозначен. возвращает RID и оценку сервера; Звоните blast_status не чаще одного раза в минуту, затем blast_results после READY. Последовательность отправляется в NCBI и не кэшируется локально; Потерянный ответ на запрос повышает blast_submission_unknown и не должен быть восстановлен автоматически. Проведите все запросы BLAST, по крайней мере, на 10 секунды, и все запросы на один и тот же RID, по крайней мере, на 60 секунды. Сохраните RID для возобновления после перезапуска. NCBI обычно сохраняет результаты в течение часов 36. Это не является гарантией удаления. Отмена, выход приложения и удаление только локальных запросов; API не имеет задокументированной операции удаленной отмены/удаления. не добавляется реестр вакансий или кэш результатов; Нормальная настойчивость разговора/ноутбука может сохранять входные и выходные данные.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `sequence` | строка | **обязательно**; Длина: `1`; Максимальная длина: `100000` |
| `molecule_type` | строка | **обязательно**строка `["nucleotide", "protein"]` |
| `database` | строка | факультативный; ЭНУМ: `["nt", "core_nt", "refseq_rna", "nr", "refseq_protein", "swissprot"]` |
| `evalue` | число | факультативный; Эксклюзивный минимум: `0`; максимум: `1000` |
| `hitlist_size` | целое число | факультативный; минимум: `1`; максимум: `100` |
| `megablast` | логическое значение | строка |
...
Один раз проверь NCBI BLAST RID. Это единственный запрос SearchInfo и никогда не проводит опросы и не ждет. Уважайте руководство NCBI, чтобы подождать, по крайней мере 60 секунды между проверками. Возвращается ожидающим, готовым, несостоявшимся или неизвестным (неизвестный или просроченный рейс). Пропускайте все запросы BLAST по крайней мере на 10 секунды и запросы на тот же RID, включая поиск результатов, по крайней мере на 60 секунды.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `rid` | строка | **обязательно**; Длина: `1`; Максимальная длина: `128` |
...
Получите ограниченные результаты для NCBI BLAST RID. Он делает один запрос и возвращает готовый = ложный, когда работа все еще ждет; Выберите json2, xml2, текстовый или табличный выход после blast_status. Результаты ограничены 2 MiB и возвращены дословно. Таблица означает NCBI Text + ALIGNMENT_VIEW=Tabular, который может включать в себя комментарии HTML, теги PRE и заголовки отчетов; Это не просто TSV или CSV. Подождите, по крайней мере, 60 секунды после последнего запроса на этот RID, включая blast_status. Никаких автоматических повторов.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `rid` | строка | **обязательно**; Длина: `1`; Максимальная длина: `128` |
| `format` | строка | факультативный; ЭНУМ: `["json2", "xml2", "text", "tabular"]` |
...
Эволюционное резюме сохранения для региона из треков UCSC phyloP / phastCons (базовые оценки по многовидовым выравниваниям). Args: chrom (chr-prefixed); старт (полуоткрытый на основе 0); конечный (исключительный); пролет, ограниченный на 100000 bp — раздвоение больше; геном (по умолчанию hg38); трек (необязательно); по умолчанию: hg19 phyloP100wayAll, hg38 phyloP100way, mm10 phyloP60wayAll, mm39 phyloP35way; другие геномы сохраняют запасной вариант phyloP100way, которого может не существовать — при необходимости укажите трек оценки от ucsc_list_tracks; положительный = сохраненный, отрицательный = быстро развивающийся; альтернативные варианты hg38 phastCons100way, phyloP30way, phastCons30way, phyloP447way, phyloP470way; hg19phastCons100way; include_values (также возврат на базу) &#123;start,end,value&#125; строки, ограниченные max_values, values_truncated флаги шапки; По умолчанию false = summary only; max_values (по умолчанию 2000). Возвращение &#123;genome, track, chrom, start, end, span_bp, n_bases_covered, coverage_fraction, mean, min, max&#125; (+ значения, values_truncated по запросу). Статистика, взвешенная по базовому пролету каждого ряда, прикреплена к окну; Необнаруженные базы ниже coverage_fraction, не нулевые. Непрофильные трассы поднимают; Также поднимается укороченный вверх по течению список строк.
...
Подать три или более последовательностей белка, ДНК или РНК в EMBL-EBI Job Dispatcher Clustal Omega для асинхронного выравнивания нескольких последовательностей. Ввод данных должен быть однозначно назван записями FASTA; Сервис принимает максимум последовательностей 4000 или 4 MiB. возвращает job_id; Сохраните его и позвоните clustalo_status до clustalo_results. Разъем запрашивает clustal_num по умолчанию, выравнивание Clustal с номерами позиций, подходящее для проверки сохраненных участков и последующего эволюционного анализа. EMBL-EBI запрашивает действительное контактное письмо. Установите контактное электронное письмо в настройках → Конфиденциальность → Поделитесь контактным электронным письмом с службами данных исследований. Потерянный ответ может представлять собой принятую удаленную работу. Не возвращайте автоматически. EMBL-EBI хранит результаты в течение ограниченного периода, контролируемого провайдером (документировано до одной недели); Это не гарантия удаления, принадлежащая приложению. job_id возобновляется после перезапуска. Этот разъем не добавляет реестр вакансий, кэш результатов или автоматическую повторную отправку, а отмена / выход приложения только останавливает локальные запросы. Последовательность отправляется в EMBL-EBI, и хост может сохранять входные данные инструмента и возвращать содержимое выравнивания в разговоре или настойчивости Notebook. Уважайте руководство по справедливому использованию EMBL-EBI: подавайте не более заданий 30 в пакете и ожидайте обработки / результатов, прежде чем отправлять больше; Этот разъем не обеспечивает перекрестного вызова дросселирования. [https://www.ebi.ac.uk/jdispatcher/docs/webservices/](https://www.ebi.ac.uk/jdispatcher/docs/webservices/) для официального контракта API.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `sequence` | строка | **обязательно**; Длина: 1; Длина: 4194304 |
| `stype` | строка | **обязательно**; enum: &#91;"protein", "dna", "rna"&#93; |
| `outfmt` | строка | факультативный; &#91;"clustal_num"&#93; |
| `title` | строка | ; шаблон: "^GC&#91;AF&#93;_&#91;0-9&#93;&#123; 9&#125;\\.&#91;0-9&#93;+$" |
| `dealign` | логическое значение | строка |
| `order` | строка | факультативный; enum: &#91;"aligned", "input"&#93; |
...
Один раз проверьте работу EMBL-EBI Clustal Omega. Это единый запрос о статусе и никогда не проводит опросы и не ждет. Позвоните ему снова, по крайней мере, через 10 секунды, пока не закончите, не ошибетесь, не ошибетесь или не найдете. Храните job_id при возобновлении после перезапуска.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Длина: 1; Длина: 128 |
...
Принесите один ограниченный файл выравнивания Clustal Omega clustal_num для завершения работы. Пройти тот же Outfmt, возвращенный clustalo_submit; Этот инструмент делает один запрос результата и возвращает содержимое выравнивания плюс предложение имени файла. Если провайдер по-прежнему сообщает QUEUED или RUNNING, он возвращается готовым: ложным с подсказкой о повторной проверке; Сбои провайдера являются ошибками и сохраняют job_id для диагностики. Результаты ограничены 8 MiB и возвращаются дословно, поэтому абонент может сохранить контент в виде файла выравнивания для проверки на консервативном месте или последующего филогенетического анализа. Никаких автоматических повторов. EMBL-EBI хранит результаты в течение ограниченного периода, контролируемого провайдером (документировано до одной недели); Это не гарантия удаления, принадлежащая приложению. job_id возобновляется после перезапуска. Этот разъем не добавляет реестр вакансий, кэш результатов или автоматическую повторную отправку, а отмена / выход приложения только останавливает локальные запросы. Последовательность отправляется в EMBL-EBI, и хост может сохранять входные данные инструмента и возвращать содержимое выравнивания в разговоре или настойчивости Notebook. Уважайте руководство по справедливому использованию EMBL-EBI: подавайте не более заданий 30 в пакете и ожидайте обработки / результатов, прежде чем отправлять больше; Этот разъем не обеспечивает перекрестного вызова дросселирования.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Длина: 1; Длина: 128 |
| `outfmt` | строка | **обязательно**; &#91;"clustal_num"&#93; |
...
Перечислите варианты митохондрий gnomAD с гетероплазматически-осознанными числами (`ac_het`, `ac_hom`, `max_heteroplasmy`) для митохондриального гена или окна координат chrM. Митохондриальный набор вызовов доступен только через GRCh38 gnomAD r3/r4 контакты набора данных: используйте набор данных `gnomad_r3` или `gnomad_r4`. Передайте ген (`gene_symbol`, как `MT-TL1`, или `gene_id`) или область (`region_start` + `region_stop`), а не оба.
...
| `dataset` | строка | необязательный `"gnomad_r4"`строка `["gnomad_r4", "gnomad_r3"]` |
...
Откройте для себя публичные секвенирования, выполняемые NCBI tax_id (включая таксоны потомков), library_strategy и / или ключевое слово в названиях исследований, экспериментов или выборок и описаниях запуска. Поставляемые фильтры сочетаются с И; Требуется хотя бы один. Таксономия описывает секвенированный организм, а не хозяина образца микробиома. Ключевое слово - это буквальная подстрочная строка, а не синтаксис запросов ENA; Двойные цитаты, обратные удары, wildcards и контрольные символы отвергаются. Включает публичные записи метагенома. Возвращает только ограниченные метаданные, а не полную когорту при усечении. Узкие фильтры для извлечения меньшего набора; Повторяющиеся звонки не являются пагинацией. Для известных присоединений INSDC используют ena_search_runs.

Поставить хотя бы один поисковый фильтр; Проконсультируйтесь с загружаемой схемой для полного набора правил.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `tax_id` | целое число | факультативный; минимум: `1`; максимум: `2147483647` |
| `library_strategy` | строка | факультативный; ЭНУМ: `["AMPLICON", "ATAC-seq", "Bisulfite-Seq", "CLONE", "CLONEEND", "CTS", "ChIA-PET", "ChIP-Seq", "ChM-Seq", "DNase-Hypersensitivity", "EST", "FAIRE-seq", "FINISHING", "FL-cDNA", "GBS", "Hi-C", "MBD-Seq", "MNase-Seq", "MRE-Seq", "MeDIP-Seq", "NOMe-Seq", "OTHER", "POOLCLONE", "RAD-Seq", "RIP-Seq", "RNA-Seq", "Ribo-Seq", "SELEX", "Synthetic-Long-Read", "Targeted-Capture", "Tethered Chromatin Conformation Capture", "Tn-Seq", "VALIDATION", "WCS", "WGA", "WGS", "WXS", "miRNA-Seq", "ncRNA-Seq", "snRNA-seq", "ssRNA-seq"]` |
| `keyword` | строка | факультативный; Длина: `1`; Максимальная длина: `200`; шаблон: `"^(?=[\\s\\S]*\\S)[^\"\\\\*?\\u0000-\\u001f\\u007f]+$"` |
| `limit` | целое число | необязательный `100`; минимум: `1`; максимум: `1000` |
...
Перечислите оригинальные файлы, представленные для одного запуска ERR/SRR/DRR, включая представленные BAM, CRAM или FASTQ, когда ENA раскрывает их. Возвращает локации FTP, представленные форматы, размеры байтов и контрольные суммы MD5 только в виде метаданных. Не загружает, не преобразует форматы, не извлекает справочные геномы и не проверяет контрольные суммы. Эти файлы представляются, а не архивный FASTQ, возвращенный ena_get_run_files, и не список сгенерированных архивом SRA-контейнеров. CRAM может потребовать сопоставительной ссылки для анализа.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `run_accession` | строка | **обязательно**; Длина: `1`; Максимальная длина: `64` |
...
Перечислите одну страницу общедоступных файлов проекта PRIDE для доступа к PXD или PRD, включая категорию файлов, размер байта, контрольную сумму и места загрузки (FTP, HTTP или Aspera). Только метаданные: не загружает файлы и не проверяет контрольные суммы. Страницы имеют нулевую основу; Сохраняйте page_size неизменным и следуйте за next_page до нуля. Заказ предоставляется PRIDE, а не снимком. Пустой список не определяет, существует ли проект или является публичным.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `project_accession` | строка | **обязательно**; Максимальная длина: `32`; шаблон: `"^(?:PXD\|PRD)[0-9]{6,}$"` |
| `page` | целое число | необязательный `0`; минимум: `0`; максимум: `1000000` |
| `page_size` | целое число | необязательный `100`; минимум: `1`; максимум: `100` |
...
Перечислите проекты рака GDC и их резюме. Фильтры четкие и ограниченные; Это открытие метаданных, а не операция загрузки данных.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `project_ids` | струна/массив | строка |
| `disease_type` | строка | ; шаблон: "^GC&#91;AF&#93;_&#91;0-9&#93;&#123; 9&#125;\\.&#91;0-9&#93;+$" |
| `primary_site` | строка | ; шаблон: "^GC&#91;AF&#93;_&#91;0-9&#93;&#123; 9&#125;\\.&#91;0-9&#93;+$" |
| `page` | целое число | факультативный; по умолчанию: 1; Минимум: 1; Максимум: 10000 |
| `page_size` | целое число | ; Длина: 1; Длина: 200 |
...
Перечислите случаи ГДК (доноры образцов) с метаданными о проектах и заболеваниях. Результаты идентифицируют случаи и не разоблачают и не загружают контролируемые данные.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `project_ids` | струна/массив | строка |
| `submitter_ids` | струна/массив | строка |
| `case_ids` | струна/массив | строка |
| `page` | целое число | факультативный; по умолчанию: 1; Минимум: 1; Максимум: 10000 |
| `page_size` | целое число | ; Длина: 1; Длина: 200 |
...
Поиск по инвентарю файлов GDC и явная маркировка каждого файла как открытого или контролируемого доступа. Обнаружение метаданных не предоставляет доступ к загрузке; Контролируемые файлы требуют соответствующей авторизации GDC.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `project_ids` | струна/массив | строка |
| `access` | строка | факультативный; по умолчанию: "all"; enum: &#91;"all", "open", "controlled"&#93; |
| `data_category` | строка | ; шаблон: "^GC&#91;AF&#93;_&#91;0-9&#93;&#123; 9&#125;\\.&#91;0-9&#93;+$" |
| `data_type` | строка | ; шаблон: "^GC&#91;AF&#93;_&#91;0-9&#93;&#123; 9&#125;\\.&#91;0-9&#93;+$" |
| `data_format` | строка | факультативный; Длина: 1; Длина: 50 |
| `file_name` | строка | факультативный; Длина: 1; Длина: 500 |
| `page` | целое число | факультативный; по умолчанию: 1; Минимум: 1; Максимум: 10000 |
| `page_size` | целое число | ; Длина: 1; Длина: 200 |
...
Восстановление метаданных для одного файла GDC UUID, включая его открытую / контролируемую классификацию доступа. Это не выполняет загрузку файла и не утверждает, что указанный файл загружается для текущего пользователя.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `file_id` | строка | **обязательно**; шаблон: "^&#91;0-9a-fA-F&#93;&#123; 8&#125;-&#91;0-9a-fA-F&#93;&#123; 4&#125;-&#91;1-5&#93;&#91;0-9a-fA-F&#93;&#123; 3&#125;-&#91;89abAB&#93;&#91;0-9a-fA-F&#93;&#123; 3&#125;-&#91;0-9a-fA-F&#93;&#123; 12&#125;$" |
...
Создайте манифест GDC Data Transfer Tool для UUID файлов 100. Возвращенный манифест - это только инвентарь; Он не загружает файлы и не обходится без авторизации с контролируемым доступом.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `file_ids` | массив строк | **обязательно**; Мини-элементы: 1; maxItems: 100; Уникальные вещи: правда |
...
Поиск публичных записей Zenodo (наборов данных, программного обеспечения и публикаций) с использованием синтаксиса строк запросов Zenodo, например. Название: "климат" или doi: "10.5281/zenodo.8435696". Вычитывает одну страницу, вплоть до записей 25, без аутентификации. По умолчанию указана только последняя версия; all_versions включает более старые версии. На следующей странице сохраняйте запросы, page_size, сортировку и all_versions без изменений. Окно поиска ограничено результатами 10,000: (страница - 1) &#42; page_size должен быть меньше 10,000; Последняя страница может быть частичной. Если pagination_limited соответствует действительности, сузьте запрос. Публичные метаданные не подразумевают открытый доступ к файлам.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `query` | строка | **обязательно**; Длина: 1; Максимальная длина: 1000; Модель: "\\S" |
| `page` | целое число | факультативный; по умолчанию: 1; Минимум: 1; Максимум: 10000 |
| `page_size` | целое число | факультативный; по умолчанию: 10; Минимум: 1; Максимум: 25 |
| `sort` | строка | факультативный; по умолчанию: "bestmatch"; enum: &#91;"bestmatch", "mostrecent"&#93; |
| `all_versions` | логическое значение | логическое значение |
...
Восстановление общедоступных метаданных Zenodo и инвентаризации файлов, представленных конечной точкой записи. Передайте десятичный идентификатор записи, а не DOI или URL. Идентификатор концепта может быть разрешен до последней версии; requested_record_id, record_id и concept_record_id отличаются друг от друга. Используйте возвращенную версию record_id для воспроизводимого поиска. description_html выше по течению HTML, не дезинфицирован. Файловые ссылки и контрольные суммы являются только метаданными: не загружается, проверка контрольной суммы или зонд доступа не выполняется. Ограниченные или запрещенные записи могут иметь общедоступные метаданные без доступных файлов. пустой список файлов не устанавливает, что на депозите нет файлов.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `record_id` | строка | **обязательно**; Максимальная длина: 20; шаблон: "^&#91;1-9&#93;&#91;0-9&#93;&#42;$" |
...
Отправьте один асинхронный поиск EMBL-EBI HMMER3. Программа выбирает phmmer (последовательность белка против базы данных последовательностей), hmmscan (последовательность белка против профилей Pfam), hmmsearch (профиль HMM / выравнивание против базы данных последовательностей) или jackhmmer (итеративный поиск удаленного гомолога). Ввод - это последовательность FASTA, профиль HMM или текст выравнивания, принятый этой программой. База данных - это имя базы данных провайдера. Факультативные пороги используют имена параметров HMMER (incE/incdomE, E/domE, incT/incdomT, T/domT); Итерации контролируют раунды джекммера. Представление не завершено: сохраните возвращенный job_id и опрос со статусом. Служба может принять работу, даже если ответ будет потерян. Никогда не возвращайте неопределенное представление автоматически.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `program` | строка | **обязательно**; enum: &#91;"phmmer", "hmmscan", "hmmsearch", "jackhmmer"&#93; |
| `database` | строка | **обязательно**; enum: &#91;"refprot", "uniprot", "swissprot", "pdb", "rp15", "rp35", "rp55", "rp75", "pfam"&#93; |
| `input` | строка | **обязательно**; Длина: 1; Длина: 200000 |
| `incE` | число | факультативный; Минимум: 0; Максимум: 10 |
| `incdomE` | число | факультативный; Минимум: 0; Максимум: 10 |
| `incT` | число | факультативный; Минимум: 0 |
| `incdomT` | число | факультативный; Минимум: 0 |
| `E` | число | факультативный; Минимум: 0; Максимум: 10 |
| `domE` | число | факультативный; Минимум: 0; Максимум: 10 |
| `T` | число | факультативный; Минимум: 0 |
| `domT` | число | факультативный; Минимум: 0 |
| `popen` | число | факультативный; Минимум: 0 |
| `pextend` | число | факультативный; Минимум: 0 |
| `mx` | строка | факультативный; enum: &#91;"BLOSUM45", "BLOSUM62", "BLOSUM90", "PAM30", "PAM70"&#93; |
| `iterations` | целое число | факультативный; Минимум: 1; Максимум: 9 |
...
Проверяйте одну работу HMMER один раз, без опроса или повторной отправки. Успех означает, что результаты доступны; ПЕНДИНГ/РУННГ означает ожидание, прежде чем снова проверить; ERROR/FAILURE/NOT_FOUND — это конечные результаты, которые никогда не означают нулевых попаданий. Сохраните точный job_id.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Максимальная длина: 36; шаблон: "↑ A-Fa-f0-9&#93;&#123; 8&#125;- &#91;А-Фа-Ф0&#93;9&#93;&#123; 4&#125;- &#91;А-Фа-Ф0&#93;9&#93;&#123; 4&#125;- &#91;А-Фа-Ф0&#93;9&#93;&#123; 4&#125;- &#91;А-Фа-Ф0&#93;9&#93;&#123; 12&#125;$" |
...
Получить один результат работы HMMER один раз. Сначала проверяется статус поставщика и не возвращает полезную нагрузку, пока работа находится на рассмотрении или не выполнена. На SUCCESS извлекает все страницы результатов, включая аннотации домена; Записи итерации джекммера возвращаются в форме массива провайдера. Сохранить результат в артефакте Notebook, потому что удержание провайдера является конечным; Пустой список матчей - это завершенный результат с нулевым ударом, отличный от ожидающей или неудачной работы.
...
Проверяйте одну работу InterProScan один раз, без повторного опроса. Подождите, по крайней мере, 10 секунды между проверками. Окончание означает, что результаты могут быть получены; Ошибка/неудача - это неудачи в работе, НЕОБЪЯВЛЕННЫЙ означает неизвестный или просроченный, никогда не имеющий нулевого результата. Храните точный job_id; Этот инструмент никогда не возвращается.

| Полное поле | Тип | Тип |
| --- | --- | --- |
| `job_id` | строка | **обязательно**; Максимальная длина: 200; шаблон: "^&#91;A-Za-z0-9&#93;&#91;A-Za-z0-9_-&#93;&#123; 0,199&#125;$" |
...
Восстановите полный отчет InterProScan TSV для работы, ограниченный 2 MiB (негабаритные отчеты терпят неудачу, никогда не усечены). Проверяет статус один раз, а затем берет TSV только для завершения. Никаких повторных опросов, опросов или повторной подачи. Сохраняйте отчет в артефакте Notebook быстро, потому что результаты провайдера истекают. TSV содержит один ряд на матч подписи; Координаты основаны на 1, а баллы специфичны для приложений. Дополнительные столбцы содержат аннотации InterPro, GO и Pathway. Пустой TSV после Финиширования означает отсутствие зарегистрированных совпадений, а не доказательств того, что белок не имеет функции.
