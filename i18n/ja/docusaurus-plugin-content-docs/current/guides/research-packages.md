...
## 受信者のバージョンをチェックする {/* #package-compatibility */}

**v0.33.0**でエクスポートされたパッケージには、`ro-crate-metadata.json`の**RO-Crate 1.1の特長**メタデータが含まれています。 ファイル名、メディアの種類、関係など、最終的なエクスポートされたスナップショットと選択した不変なファイルについて説明します。 再エクスポートは、新しいスナップショットの参照を再構築します。

これらの新しいエクスポートを開くには、**v0.33.0 以降対応リーダー** を使用します。必要な `ro-crate` 機能が宣言されます。 インポートする前に、古い受信アプリを更新します。 メタデータのリネームや削除は、互換性の修正ではありません。 移行せずに、古いパッケージを既存のパッケージが読みやすくなります。

このメタデータは`.science`パッケージ内で移動します。 計算を再実行したり、認証を付与したり、[再現性は、アーティファクト版で確認します](reproducibility.md)を交換したりしません。
