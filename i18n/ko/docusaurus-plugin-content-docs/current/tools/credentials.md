...
| OpenAlex | 문학의 OpenAlex 작업에 대한 옵션 API 키 | 입력된 키를 검증하고 저장하고 바인딩 된 쿼리를 만듭니다. |
...
## 선택 사항 OpenAlex 키 구성 {/* #openalexs-actual-missing-key-flow */}

v0.33.1에서, OpenAlex 쿼리 더 이상 API 키가 필요 없습니다. 작은 쿼리로 시작; 서비스 요금 제한, 인증 및 액세스 정책은 여전히 적용됩니다. keyless 요청을 허용하는 것은 무제한 사용 또는 성공적인 응답을 약속하지 않습니다.

자신의 키를 사용하려면 **Settings → Credentials → OpenAlex** (또한 **Manage credentials**을 통해 문학 그래프에서 사용할 수 있습니다), **API key**를 입력, **Validate**을 선택, 다음 **Save** 성공적인 유효성 검사 후. 키는 `api.openalex.org`에서만 사용됩니다. **Remove key**는 기존 키 제거; 교체 필드는 저장 된 비밀을 밝혀지지 않습니다.

해결 시스템 자격 증명 오류를 저장하기 전에. 429 응답을 위해, 열쇠를 assuming 대신 서비스의 할당량 및 재량 지도를 검사합니다.
...
OpenAlex 열쇠는 선택적입니다. OAuth 연결관은 아직도 지명한 서비스의 표시에서 완료합니다. 동일한 작은 쿼리를 재발하기 전에 표시된 인증 오류를 해결합니다.
...
OpenAlex 및 NCBI 자격 양식은 공식 키 페이지에 링크합니다. 서비스로 계정 단계 완료, 양식에 반환, 다음 유효성 검사 및 예정된 키를 저장. 링크를 열거나 키를 저장할 수 없습니다; 키가 필요한 경우 서비스 및 운영에 따라 달라집니다.